...
if (isset($_POST['edit_id'])) {
    ...
    header("Location: ../views/$type.php?news_id=$id");
    exit();
}
...
<h2 class="text-center">แก้ไข ประกาศ</h2>                   
...
<form role="form" name="form1" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
    ...
    <div class="checkbox">
        <label>
            <input type="checkbox" name="new" value="y"<?php echo $chk; ?>> ข่าวใหม่/แสดงหน้าแรก
        </label>
    </div>
    <p>&nbsp;</p>
    <button type="submit" class="btn btn-default">Submit</button> | <a href="../views/notice.php?news_id=<?php echo $p['id']; ?>" title="Cancel"><strong>Cancel</strong></a>
</form>
